...
void aes128_init(AES128_Context *ctx, const uint8_t key[16]) {
    ...
    while (bytes_generated < 176) {
        ...
        for (int i = 0; i < 4; ++i) {
            ctx->round_keys[bytes_generated] = ctx->round_keys[bytes_generated - 16] ^ temp[i];
            bytes_generated++;
        }
    }
}
...
static uint8_t gmul(uint8_t a, uint8_t b) {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        a <<= 1;
        if (hi_bit_set) a ^= 0x1b; // Rijndael's Galois field polynomial
        b >>= 1;
    }
    return p;
}
